...
Окружение — именованный набор переменных (ключ → значение), который становится доступен в PHP-сниппете через массив `$env`. Удобно для параметризации сниппетов без изменения кода.

```php
// Если выбрано окружение с переменными host=db.local, port=3306
echo $env['host']; // db.local
echo $env['port']; // 3306
```
